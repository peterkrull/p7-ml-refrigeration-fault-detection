...
plt.savefig(sys.path[0] + "/LDA_SVM_sceeplot.pdf")
plt.show()
...
#Make LDA transformation
LDA = lda.LDA_reducer(trn_std, 6, 'target')
trn_red = LDA.transform(trn_std)
vld_red = LDA.transform(vld_std)
tst_red = LDA.transform(tst_std)
...
f = open(sys.path[0] + "/lda6_svm_grid_search.txt", 'w')
f.write(str(datetime.now()) + "\n\n")
f.write(str(clf.cv_results_))
f.write("\n\n" + str(clf.best_estimator_))
f.close()
...
cv_log = pd.DataFrame.from_dict(clf.cv_results_)
f = open(sys.path[0] + "/lda6_svm_grid_search_log.json", 'w')
f.write(cv_log.to_json())
f.close()

#Evaluate classifier using validation data
vld_pred = clf.predict(vld_red.drop('target', axis = 1))
confusion_matrix.confusion_matrix(vld_red['target'], vld_pred, save_fig_name = sys.path[0] + "/LDA6_SVM_confmat_vld.pdf", title = "LDA6-SVM, validation data")

#Evaluate classifier using testing data
tst_pred = clf.predict(tst_red.drop('target', axis = 1))
confusion_matrix.confusion_matrix(tst_red['target'], tst_pred, save_fig_name = sys.path[0] + "/LDA6_SVM_confmat_tst.pdf", title = "LDA6-SVM, test data")
