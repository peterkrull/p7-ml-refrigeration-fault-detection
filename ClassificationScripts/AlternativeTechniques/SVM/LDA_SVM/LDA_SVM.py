...
gamma_params = [10**x for x in np.linspace(-3, 1, 41)]
C_params = [10**x for x in np.linspace(2, 7, 51)]
#gamma_params = [10**x for x in np.linspace(-3, 1, 5)]
#C_params = [10**x for x in np.linspace(2, 7, 6)]
#Execute gridsearch
clf = GridSearchCV(svc, {'C' : C_params, 'gamma' : gamma_params}, n_jobs= -1, verbose=3.1, cv =GroupKFold(n_splits=5))
clf.fit(trn_red.drop('target', axis = 1), trn_red['target'], groups=trn_set)
