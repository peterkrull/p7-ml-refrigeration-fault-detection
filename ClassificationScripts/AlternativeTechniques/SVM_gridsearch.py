...
if __name__ == "__main__":
    ...
    f.close()
    dump(clf,sys.path[0] + '/optimum_Svm/GridSearchBest.joblib')
    ...
    confusion_matrix.confusion_matrix(test_data['target'],tst_pred, save_fig_name = sys.path[0] + "/optimum_Svm/optimum_svm_tst_kfold.pdf")

    print("Best estimator = " + str(clf.best_estimator_))

    f = open(sys.path[0] + "/optimum_Svm/svm_grid_search_kfold.txt", 'w')
    f.write(str(datetime.now()) + "\n\n")
    f.write(str(clf.cv_results_))
    f.write("\n\n" + str(clf.best_estimator_))
    f.close()
    ...
    cv_log = pd.DataFrame.from_dict(clf.cv_results_)
    f = open(sys.path[0] + "/optimum_Svm/svm_grid_search_log_kfold.json", 'w')
    f.write(cv_log.to_json())
    f.close()

    dump(clf,sys.path[0] + '/optimum_Svm/GridSearchBest_kfold.joblib')