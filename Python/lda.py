...
class LDA_classifier:
    ...
    def __init__(self, X : 'pd.ndarray', y : 'pd.ndarray', p: 'pd.ndarray' = None) -> None:
        ...
        self.P = np.ascontiguousarray( [len(X[y == c]) for c in self.classes], dtype = float ) if p == None else p 
    ...
    def classifier(self,X):
            ...
            X = np.array(X)
            return np.argmax([ lda_discriminant_function(X,S,m,p) for m,p in zip(self.M,self.P) ], axis=0)
...
class QDA_classifier:
    ...
    def classifier(self,X):
            ...
            X = np.array(X)
            return np.argmax([ qda_discriminant_function(X,s,m,p) for m,s,p in zip(self.M,self.S,self.P) ], axis=0)
